...
func (c *Map2dPersistenceFixture) testCreateMaps(t *testing.T) {
	...
	m, err := c.persistence.Create(context.Background(), *c.MAP1)
	assert.Nil(t, err)
	assert.NotNil(t, m)
	...
	m, err = c.persistence.Create(context.Background(), *c.MAP2)
	assert.Nil(t, err)
	assert.NotNil(t, m)
	...
	m, err = c.persistence.Create(context.Background(), *c.MAP3)
	assert.Nil(t, err)
	assert.NotNil(t, m)
...
func (c *Map2dPersistenceFixture) TestCrudOperations(t *testing.T) {
	...
	m, err := c.persistence.Update(context.Background(), map1)
	assert.Nil(t, err)
	assert.NotNil(t, m)
	...
	// Delete the map
	m, err = c.persistence.DeleteById(context.Background(), map1.Id)
	assert.Nil(t, err)
	assert.NotNil(t, m)
	assert.Equal(t, map1.Id, m.Id)

	// Try to get deleted map
	m, err = c.persistence.GetOneById(context.Background(), map1.Id)
	assert.Nil(t, err)
